...
//				   sql += " where rnum >= ? and rnum <= ?";		// between 대신 연산자 활용하여 작성 가능
	...
	public int delete(BoardDataBean board, String path) {
		...
		try {
			...
			// delete SQL문 실행 -> 삭제된 데이터 개수를 반환
			result = pstmt.executeUpdate();
			
			
			/** 업로드 한 첨부파일이 존재하는 경우, 해당 파일을 삭제하는 코드 필요 */
			if(board.getUpload() != null) {
				/* 업로드 한 첨부파일이 존재하는 경우, 해당 디렉토리 모든 파일을 구해와서
				   db에 업로드 된 파일의 이름과 일치한다면, 해당 파일을 삭제 */
				
				// 지정된 path에 해당하는 디렉토리를 나타내는 File 객체를 생성
				File file = new File(path);
				
				// upload 디렉토리 내 모든 파일을 구해옴
				File[] fileList = file.listFiles();
				
				// 각 파일에 대한 순회
				for(int i=0; i<fileList.length; i++) {
					// upload 디렉토리에 저장된 파일 중에서,
					// db에 저장된 파일(board.getUpload())을 지정된 경로(path)에서 삭제
					if(fileList[i].getName().equals(board.getUpload())) {
						// 파일 시스템에서 해당 파일을 영구적으로 삭제
						fileList[i].delete();
					}
				}
			}
