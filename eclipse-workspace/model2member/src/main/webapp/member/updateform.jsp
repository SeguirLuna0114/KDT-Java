...
<script>
	$(document).ready(function() {
		...
		// select 옵션
		/** 핸드폰 앞자리 선택
		 	id값이 phone1인 태그를 가져옴 
		 	-> 해당 태그의 후손태그(option태그) 중 value값이 'member.phone1'인 태그를 가져옴 */
		...
		// 체크박스 - 취미 선택
		// 반복문(<forEach>)을 사용하여 h라는 컬렉션(또는 배열)에서 각 값(s)을 반복
			/* value 속성을 ${s}로 설정하여 해당 값에 해당하는 체크박스를 선택
			   attr("checked", true) 메소드를 사용하여 체크박스를 선택 */
	...
	<form method="post" action="<%=request.getContextPath()%>/Update.do">
		...
		<table border=1 width=500 align=center>
			...
			<tr>
				<td>ID</td>
				<td>
					<!-- 1. scriptlet을 사용하여 출력  -->
				<% 
					MemberDTO mem = (MemberDTO)request.getAttribute("member");
				%>	
					<%=mem.getId() %><br>
					<!-- 2. EL표기 사용하여 출력(간결함) : 공유되는 name값.필드명 -->
					${member.id }
				</td>
			</tr>
			<tr>
