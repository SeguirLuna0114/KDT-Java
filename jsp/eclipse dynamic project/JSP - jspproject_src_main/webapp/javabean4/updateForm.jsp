...
<%
	// primary key인 id값을 활용해서 회원 조회
	String id = request.getParameter("id");
	
	LogonDBBean manager = LogonDBBean.getInstance();
	// id와 일치하는 회원 데이터를 DTO데이터타입으로 가져옴
	...
	<form name="regForm" method="post" action="updatePro.jsp" onsubmit="return inputCheck()">
		...
		<table width="500" border="0" cellspacing="0" cellpadding="2" align="center">
			...
			<tr>
				...
				<td width="200" bgcolor="<%=value_c%>">
					<input type="text" name="name" size="20" value="<%=db.getName()%>">
				</td>
			</tr>
			<tr>
				...
				<td width="200" bgcolor="<%=value_c%>">
					...
					-
					<input type="text" name="jumin2" size="7" maxlength=7 value="<%=db.getJumin2()%>">
				</td>
			</tr>
			<tr>
				<td width="100" bgcolor="<%=title_c%>">이메일</td>
				<td width="200" bgcolor="<%=value_c%>">
					<input type="text" name="email" size="30" value="<%=db.getEmail()%>">
				</td>
			</tr>
			<tr>
				<td width="100" bgcolor="<%=title_c%>">블로그</td>
				<td width="200" bgcolor="<%=value_c%>">
					<input type="text" name="blog" size="20" value="<%=db.getBlog()%>">
				</td>
			</tr>
			<tr>
